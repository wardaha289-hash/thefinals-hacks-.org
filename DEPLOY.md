# Deploy thefinalshacks.org

Step-by-step guide to deploy the THE FINALS Hacks static site to **thefinalshacks.org** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **thefinalshacks.org** DNS
...
3. Configure build settings:
   - **Project name:** `finalsscheats` (existing) or create a new project
...
This runs `wrangler pages deploy dist --project-name=bestfinalscheats` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **thefinalshacks.org** as the primary custom domain on the Pages project.

### Apex (thefinalshacks.org)
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.thefinalshacks.org/*`
   - **Target:** `https://thefinalshacks.org/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`thefinalshacks.org`, `.net`, `.com`), and legacy path redirects.
...
- `https://thefinalshacks.org/`
- `https://thefinalshacks.org/es/`
- `https://thefinalshacks.org/the-finals-hacks/`
- `https://thefinalshacks.org/finals-aimbot/`
- `https://thefinalshacks.org/sitemap.xml`
- `https://thefinalshacks.org/robots.txt`

Verify redirects:

- `http://thefinalshacks.org` → `https://thefinalshacks.org` (301)
- `https://www.thefinalshacks.org` → `https://thefinalshacks.org` (301)
- Legacy domains (e.g. `thefinalshacks.org`) → `https://thefinalshacks.org` (301)
...
2. **Add property** → choose **Domain** → enter `thefinalshacks.org`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://thefinalshacks.org/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `thefinalshacks.org` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/the-finals-hacks/`)
   - Key landing pages (`/finals-aimbot/`, `/finals-esp/`, `/the-finals-hacks-2026/`, etc.)
...
- [ ] Custom domain `thefinalshacks.org` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `thefinalshacks.org`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://thefinalshacks.org`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/the-finals-hacks/` requested for indexing
