...
const sourcePath =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-finalshacks/assets/c__Users_Aman_AppData_Roaming_Cursor_User_workspaceStorage_5fc01ad59007d6b1ec3564ba48b04eb7_images_image-4be7ee07-5934-4878-8eb6-1fb2f3143ab4.png';
...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'the-finals-hacks-logo.png'), logoPng);
	console.log('Wrote public/images/the-finals-hacks-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'the-finals-hacks-logo.webp'), logoWebp);
	console.log('Wrote public/images/the-finals-hacks-logo.webp');
}
...
async function generateWebManifest() {
	const manifest = {
		...
		description: 'Undetected thefinals hacks — ESP, aimbot, radar and for PC',
		start_url: '/',
		display: 'standalone',
