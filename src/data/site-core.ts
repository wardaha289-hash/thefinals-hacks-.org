...
import { ROUTE_KEYWORDS, SITE_PURPOSE } from './seo-config';
...
const copyDefaults = {
	...
	summary:
		'{brand} is an {game} cheat package for Windows PC. Includes ESP, soft aim, and radar, with {antiCheat} maintenance after patches.',
...
const seoDefaults = {
	homeTitle: '{brand} | {primaryKeyword}',
	homeDescription:
	...
	blogDescription: ' and notes for {game} — meta tips, ESP, aimbot, and {antiCheat} updates.',
	forumTitle: '{game} Forum | {brand}',
	forumDescription:
		'Setup, features, and configuration threads for {brand} — ESP, soft aim, radar, and performance on Windows PC.',
} as const;
...
/** Prefer brand.keywords — home intent only; not emitted as a meta keyword dump. */
export const seoKeywords = ROUTE_KEYWORDS.home;
...
export const productInfo = {
	...
	features: {
		esp: [
			'Player ESP with box ESP overlays',
			'Team ESP and bot ESP filters',
			'Skeleton ESP and snaplines',
			'Health ESP, name ESP, and distance ESP',
			'Visibility check through destructible cover',
			'Toggleable ESP categories',
		],
		aimbot: [
			'Aimbot with aim smoothing',
			'FOV circle and crosshair overlay',
			'Bone selection and target selection',
			'Distance limit controls',
			'Keybinds mid-match',
			'Per-weapon profiles',
		],
		radar: ['2D radar overlay', 'Adjustable range', 'Works in Cashout and Quick Cash'],
		general: [
			'Menu settings and language settings',
			'FPS limit and CPU support on Windows PC',
