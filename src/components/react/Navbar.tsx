...
import FinalsMark from '../FinalsMark';
...
const icons: Record<string, string> = {
	home: 'M5.8 10.4 12 6.2l6.2 4.2V17.6a1.1 1.1 0 01-1.1 1.1H6.9a1.1 1.1 0 01-1.1-1.1v-7.2zM10 17.6v-3.8h4V17.6',
	hacks:
	...
	forums:
		'M5.2 6.8h13.6v8.1a1.1 1.1 0 01-1.1 1.1H6.3a1.1 1.1 0 01-1.1-1.1V6.8zm3.2 4.2h7.2M8.6 10.2h6.8',
};
...
function NavbarInner({
	...
	const isActive = (href: string, id?: string) => {
		if (id === 'home' || href === '/' || href === homeHref) {
			return currentPath === '/' || currentPath === homeHref || currentPath === `/${locale}/`;
		}
		if (id === 'forums' || href === '/forum/') {
			return currentPath === '/forum/' || currentPath.startsWith('/forum/');
		}
	...
	return (
		<header className={`site-header${scrolled || open ? ' is-scrolled' : ''}${open ? ' is-open' : ''}`} data-nav>
			<div className="shell site-header__bar">
				<a className="site-brand" href={homeHref} data-edit="navBrand" aria-label={siteName}>
					<span className="site-brand__text">{navBrand}</span>
					<FinalsMark className="site-brand__mark" />
				</a>
				<nav className="site-nav site-nav--bubble" aria-label={t('nav.primaryAria')}>
					{navLinks.map((item) => (
